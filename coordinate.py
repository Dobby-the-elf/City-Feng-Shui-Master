...
for m in range(1, 7):
    data = pd.read_csv("./source/2021年" + str(m).zfill(2) + "月臺南1999派工案件.csv").to_numpy()  # modify
    date = data[:, 2]
    event = data[:, 4]
    region = data[:, 6]
    ...
    for i in range(len(date)):
        if type(date[i]) != float:
            day = date[i].replace("2021/" + str(m) + "/", "")  # modify

            kind = -1  # 0:交通事故 1:自然環境 2:公共安全 3:生活機能
    ...
    for d in range(31):
        ...
        for i in range(77):
            for j in range(68):
                result.append([i + 40, j + 7, record[d][i][j][0], record[d][i][j][1], record[d][i][j][2], record[d][i][j][3]])
        ...
        os.mkdir("./temp/2021_" + str(m) + "_" + str(d + 1))  # modify
        with open("./temp/2021_" + str(m) + "_" + str(d + 1) + "/event_full.csv", "w", newline='') as csvfile:  # modify
            writer = csv.writer(csvfile)
            writer.writerow(["grid_id", "row", "col", "event1_num", "event2_num", "event3_num", "event4_num"])
        pd.DataFrame(result).to_csv("./temp/2021_" + str(m) + "_" + str(d + 1) + "/event_full.csv", mode="a", header=False)  # modify

        result = np.delete(result, remove, axis=0)
        # os.mkdir("./temp/2021_" + str(m) + "_" + str(d + 1))  # modify
        with open("./temp/2021_" + str(m) + "_" + str(d + 1) + "/event.csv", "w", newline='') as csvfile:  # modify
            writer = csv.writer(csvfile)
            writer.writerow(["grid_id", "row", "col", "event1_num", "event2_num", "event3_num", "event4_num"])
        pd.DataFrame(result).to_csv("./temp/2021_" + str(m) + "_" + str(d + 1) + "/event.csv", mode="a", header=False)  # modify
